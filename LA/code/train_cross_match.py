...
import time
import yaml
from utils.thresh_helper import ThreshController, DropRateController
...
parser.add_argument("--weight_schedule", type=str, default='linear',
                    choices=['fixed', 'linear', 'cosine', 'exp'])
args = parser.parse_args()

train_data_path = args.root_path
snapshot_path = "../model_tiaocan/" + args.exp + "/"
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (
            ...
            # 动态计算 current_iter 和 eta
            current_iter = epoch_num * len(trainloader_u) + i_batch
            total_iters = len(trainloader_u) * 361
            
            # 计算 eta
            if args.weight_schedule == 'fixed':
                eta = args.eta
            elif args.weight_schedule == 'linear':
                eta = current_iter / total_iters
            elif args.weight_schedule == 'cosine':
                eta = 0.5 * (1 + np.cos(np.pi * (current_iter / total_iters)))
            elif args.weight_schedule == 'exp':
                eta = 1 - np.exp(-5 * current_iter / total_iters)
            ...
            with torch.no_grad():
                ...
                _, _, _,_, pred_complete_s = net(
    torch.cat((img_u_s1, img_u_s2)), need_fp=True, input_type="strong")
            ...
            preds, preds_fp = net(torch.cat((img_x, img_u_w)),  need_fp=True,
                input_type="normal")
            ...
            loss_u_w_fp = dice_loss(pred_u_w_fp.softmax(dim=1)[:, 1, :, :, :], mask_u_w == 1,
                                         ignore=(conf_u_w < conf_thresh).float())


            loss_complete_dice = dice_loss(
                pred_complete_s.softmax(dim=1)[:, 1, :, :, :],
                mask_u_w == 1,
                ignore=(conf_u_w < conf_thresh).float(),
            )
            loss_u_w_fp =(1-args.eta) *(loss_u_w_fp+loss_complete_dice)
            ...
            writer.add_scalar("loss/loss_u_w_fp", loss_u_w_fp, iter_num)
